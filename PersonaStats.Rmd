...
```{r}
pairs_all %<>% 
  mutate(education_flag = str_detect(pairs, "education"),
         leadership_flag = str_detect(pairs, "management|leadership"),
         admin_flag = str_detect(pairs, "management|Program"),
         software_flag = str_detect(pairs, "Computational work|Cloud|Engineering|"),
         clinical_flag = str_detect(pairs, "Clinical"))
```


```{r}
pairs_all %>% select(ends_with("flag")) %>% colSums() %>% as.data.frame()
```

Do we want a column where admin is only flagged if leadership isn't flagged? Do we want this to be simplified to see the flags as pairs like with original descriptions?
