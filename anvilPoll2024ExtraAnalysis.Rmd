...
kowPlotSupp <- ggplot(dfForPlotKOWSupp,
                      aes(y = factor(KindOfWork, levels = factorLevel),
                          fill = "#25445A")) +
  geom_bar(stat="count") +
  theme_bw() +
  ...
  xlab("Count") +
  xlim(0, nrow(resultsTidy)) +
  scale_fill_manual(values = "#25445A") +
  geom_text(
    aes(label = sprintf('%d (%s)',
                        after_stat(count),
                        pct_format(after_stat(count) / nrow(resultsTidy))
                        )
        ),
    stat = 'count',
    nudge_x = 5,
...
ggsave(here("plots/2024/kindOfWorkBar.png"), width = 6.25, height = 3.86, units = "in")
...
The above plot was for all survey responses. Here we want to focus on institutions represented by just returning users of AnVIL.

<details><summary>Description of variable definitions and steps</summary>

We first select rows/responses that are just from returning users. Then we prepare the data and plot following the same scheme as above.
...
resultsTidy %>%
  ...
  ggtitle(bquote('Institutional Affilition for' ~ bold('Returning User') ~ 'Respondents')) +
  annotation_custom(textGrob(paste("There are\n", nrow(unique(resultsTidy[which(resultsTidy$UserType == "Returning User"), "InstitutionalAffiliation"]))  ,"\nunique institutions"), gp=gpar(fontsize=8, fontface = "bold")),xmin=5.5,xmax=5.5,ymin=3,ymax=3) +
  coord_cartesian(clip = "off")

ggsave(here("plots/institutionalAffilition_returningUserResponses.png"))
```
...
Note that the x- and y-axis labels are turned off since this will be the top plot when combined, also simplified the title to just say Returning Users. Turned off the legend.
...
```{r}
institutionTypeReturning <- resultsTidy %>%
  filter(UserType == "Returning User") %>%
  ...
  xlab("Count") +
  #xlab("") +
  ggtitle(bquote(bold("Returning Users"))) +
  coord_cartesian(clip = "off") +
...
institutionTypeReturning

#ggsave(here("plots/institutionalType_returningUserResponses.png"), plot = institutionTypeReturning)
```
...
Wanted to sync the colors between the returning and potential institutional types and so used the Set3 palette for scale_fill_brewer as it has 12 colors (and need 9 for returning users) and it seemed more accessible than the Paired palette. To see the hex codes that were assigned to the shared institution types in this plot, I used the `scales` library and `brewer_pal(palette = "Set3")(9)`

Turned off the y-axis label, but kept the x-axis label since this will be the bottom plot when combined with the returning user version (`institutionTypeReturning`). Also used `xlim` to sync the x-axis limits between the two.
...
Combined the two plots for institutional type (`institutionTypeReturning` and `institutionTypePotential`) using patchwork, stacking them on top of each other (`/`) and using `plot_layout` to set the heights since there are more institution types for potential users than returning users and therefore want returning users to be shorter than default.


```{r}
combined_plot <-  institutionTypePotential / institutionTypeReturning + plot_layout(heights = unit(c(4, 2),'cm')) + plot_annotation("What institution are you affiliated with?")
...
AnVIL_only <-
  setdiff(toPlotToolKnowledgeSeparateBR[toPlotToolKnowledgeSeparateBR$UserType == "Returning Users" &
...
PlotToolKnowledge_potential_user_score <-
  ...
  geom_point(data = toPlotToolKnowledgeSeparateBR[toPlotToolKnowledgeSeparateBR$UserType == "Returning Users",],
...
simplerPlot + geom_point(data = onAnVIL, aes(x=avgScore.x,y=Tool,colour="#C77CFF")) +
  scale_color_manual(
    values = c("#F8766D", "#00BFC4", "#C77CFF"), labels = c("Potential Users", "Returning Users", "Returning User Ratings\nfor related AnVIL tools")) + theme(legend.title = element_blank())
...
Both of these give us confidence that returning and potential user counts for reported experience level in these research areas show similar distributions. So we'll go ahead and plot it without splitting out `UserType`.
...
```{r}
resultsTidy %>%
  select(starts_with(c("ReturningRank", "PotentialRank"))) %>%
...
Here, we just want all of the numerical ranks in one column and we can have additional columns that describe if that rank was from a returning or potential user and which feature it corresponds to.

So to make a dataframe `densitydf`, we

* start by selecting the columns of interest from `resultsTidy` using `select(starts_with(c("PotentialRank", "ReturningRank")))
...
* drop rows that have na with `drop_na()` since as described earlier not every survey respondent was asked each question; e.g., if they were a returning user they weren't asked as a potential user.
* Then we `separate` the `name` column on the word "Rank" to remove the `name` column we just made but then make two new columns (`Usertype` and `Feature`) where `Usertype is either "Returning" or "Potential", and the Features are listed in the code below, because...
...
densitydf <- resultsTidy %>%
    select(starts_with(c("PotentialRank", "ReturningRank"))) %>% pivot_longer(cols = everything()) %>% drop_na() %>%
  separate(name, c("Usertype", "Feature"), sep = "Rank", remove = TRUE) %>%
    mutate(Feature =
           ...
           Usertype =
             case_when(Usertype == "Returning" ~ "Returning Users",
...
To do this we
  ...
  * Select the relevant columns from `resultsTidy`, specifically using  `select(starts_with(c("PotentialRank", "ReturningRank")))`
  ...
  * Then we `separate` the `name` column on the word "Rank" to remove the `name` column but then make two new columns (`Usertype` and `Feature`) where `Usertype is either "Returning" or "Potential", and the Features are listed in the code below, because...
  * We then use a `case_when` within a `mutate()` to fill out those features so they're more informative and show the choices survey respondents were given.
  * we add another `case_when` within that `mutate` to add the word "Users" to the `Usertypes` column values.
...
```{r}
countdf <- resultsTidy %>%
    select(starts_with(c("PotentialRank", "ReturningRank"))) %>%
  pivot_longer(cols = everything()) %>%
  drop_na() %>%
...
We use the `position = "fill"` argument in `geom_bar()` so that it's a percent stacked bar instead of raw counts (since returning and potential users had a different number of respondents)
...
How many repositories did users select/report?

```{r}
numReposPlot <- resultsTidy %>% 
  mutate(numRepos = str_count(RepositoriesDMS, ",|&")+1) %>% #count commas or an and sign and add one since 2 commas means three responses
  filter(!str_detect(RepositoriesDMS, "I'm not sure")) %>% #remove rows where they responded I'm not sure what DMS policy is no matter how many things they selected
  ggplot(aes(y = factor(numRepos))) + 
  geom_bar(fill = "#25445A") +
  ggtitle("What repositories are you considering to share data\n(for example, to comply with the NIH DMS Policy)?")
...
toPlotFundingSource %>%
  mutate(UserType = case_when(
    UserType == "Returning User" ~ "Returning",
