...
genomicsExpPlot_replot <- ggplot(experienceDf, aes(x=n, y=experienceLevel, fill=UserType)) +
  facet_wrap(~researchType, ncol=3) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(
                  aes(label = after_stat(x), group = experienceLevel),
                  stat = 'summary', fun = sum, hjust = -1, size=2
                ) +
      coord_cartesian(clip = "off")

genomicsExpPlot_replot %<>% stylize_bar()
