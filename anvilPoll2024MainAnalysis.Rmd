...
genomicsExpPlot_replot %<>% stylize_bar(xlabel = "Percent of Respondents", usertypeColor = FALSE, sequentialColor = TRUE)
...
forumPlot <- ggplot(resultsTidy, aes(y = forumAwareness, fill = UserType)) +
  geom_bar() +
  ggtitle("Have you ever read or posted in our AnVIL Support Forum?") +
  #stat_summary(aes(label = after_stat(..count..)), fun = "sum", geom = "text") +
  geom_text(
    aes(label = after_stat(..count..)), stat = "count", position = "stack", hjust = -1, size=2) + 
    coord_cartesian(clip = "off")
...
whereRunPlot <- resultsTidy %>%
  ...
  geom_text(
    aes(label = after_stat(x), group = WhereAnalysesRun),
    stat = 'summary', fun = sum, hjust = -1, size=2
) + coord_cartesian(clip = "off")
...
```{r}
nonOtherResponses <- read_delim(here("data/codebook.txt"), #read in the codebook
                                delim = "\t", 
...
dmsPlot <- resultsTidy %>%
  ...
  summarize(count = n()) %>%
  ggplot(aes(x = count, 
             y = reorder(RepositoriesMut, count),
             fill = UserType)) +
  geom_bar(stat = "identity") +
...
dmsPlot %<>% stylize_bar(ylabel = "Data Repository")
